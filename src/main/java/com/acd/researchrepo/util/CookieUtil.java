...
public class CookieUtil {
  ...
  /** Sets the refresh token cookie using modern ResponseCookie to support SameSite attributes. */
  ...
  /**
   * Internal helper to build and add the cookie header. Logic: In "prod", use SameSite=None and
   * Secure=true for cross-site (GitHub -> Tailscale).
   */
  private void addCookie(HttpServletResponse response, String value, int maxAge) {
    ...
    ResponseCookie cookie =
        ResponseCookie.from(refreshTokenName, value)
            .httpOnly(true)
    ...
    log.debug(
        "Cookie set: Name={}, MaxAge={}, Secure={}, SameSite={}",
        refreshTokenName,
        maxAge,
        cookie.isSecure(),
        cookie.getSameSite());
